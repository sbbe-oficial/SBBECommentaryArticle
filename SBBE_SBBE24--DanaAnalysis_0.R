...
make_map_plot("./SBBEPlots/SBBEArticleMap_Fig2-EN_NoLines.pdf", x_labels = xlabel_EN, y_labels = ylabel_EN,
              region_label_column = "name_region_EN", filter_abroad_only = FALSE, format = "pdf")


# Cria uma coluna de geometria vazia (sfc)
empty_geometry <- st_sfc(NA)


# Cria um data frame vazio para o painel vazio
empty_row <- data.frame(Variable = NA,
                        name_region = "Empty",
                        year = 2019,
                        Percentage = NA,
                        Stats = "Empty",
                        ...
                        Longitude.SP = NA,
                        Latitude.SP = NA,
                        name_region_EN = "Empty",
                        name_region_Bilingual = NA,
                        name_region_Bilingual_2 = NA,
                        Division_Bilingual = NA)


# Convert to sf (if geometry is NA, it will be an empty geometry)
empty_sf <- st_sf(empty_row, geometry = empty_geometry, crs = st_crs(fulldf_map))


fulldf_map <- rbind(fulldf_map, empty_sf)



# 2. Overlay onto the base map grid
Final_Composite_Plot <- ggdraw(Base_Map) +
  draw_image(
    "./SBBEPlots/Temp_Institutions_Plot.png", 
    x = 0.2,       # Centers on Column 2
    y = 0.05,       # Centers on Row 2
    width = .5,   # Fixed box size
    height = .5
  )

# 3. Export complete figure to PDF
ggsave(
  "./SBBEPlots/SBBEArticleMap_Fig2_Complete.pdf", 
...
# Creates Institutions bilingual plot (scaled down for compositing) ~
Institutions_Plot <- ggplot(Circular, aes(x = as.factor(ID), y = Percentage * 100, fill = Region)) +
  geom_bar(stat = "identity", alpha = 1) +
  scale_fill_manual(values = c("#1b9e77", "#fdb462", "#fb8072", "#bebada", "#80b1d3", "#c994c7")) +
  
  # Gridlines using base_data_Circular
  geom_segment(data = base_data_Circular, aes(x = start, y = 5, xend = end, yend = 5), 
               colour = "#000000", alpha = 1, linewidth = .25, linetype = 4, inherit.aes = FALSE) +
  geom_segment(data = base_data_Circular, aes(x = start, y = 10, xend = end, yend = 10), 
               colour = "#000000", alpha = 1, linewidth = .25, linetype = 4, inherit.aes = FALSE) +
  geom_segment(data = base_data_Circular, aes(x = start, y = 15, xend = end, yend = 15), 
               colour = "#000000", alpha = 1, linewidth = .25, linetype = 4, inherit.aes = FALSE) +
  
  # Grid percentage labels (scaled down from 26 -> 3.5)
  annotate("text", x = rep(max(Circular$ID, na.rm = TRUE), 3), y = c(5, 10, 15), 
           label = c("5%", "10%", "15%"), family = "Cormorant", size = 3.5, fontface = "bold", 
           color = "#000000", hjust = 1) +
  ...
  theme(panel.background = element_rect(fill = "#ffffff"),
        ...
        panel.border = element_blank(),
        legend.position = "none",
        plot.title = element_text(family = "Cormorant", size = 14, face = "bold", hjust = .5, margin = margin(t = 5)),
        plot.subtitle = element_text(family = "Cormorant", size = 12, colour = "#555555", face = "bold", hjust = .5, margin = margin(t = 2)),
        axis.text = element_blank(),
        axis.title = element_blank(), 
        axis.ticks = element_blank()) +
  coord_polar() +
  
  # Institution arc labels (scaled down from 30 -> 3.5)
  geom_text(data = label_data_Circular, aes(x = ID, y = Percentage * 100 + 6, 
                                            label = Variable, hjust = hjust), family = "Cormorant", size = 3.5, 
            color = "#000000", fontface = "bold", angle = label_data_Circular$angle, inherit.aes = FALSE) +
  
  geom_segment(data = base_data_Circular, aes(x = start, y = -5, xend = end, yend = -5), 
               colour = "#000000", alpha = 1, linewidth = .6, inherit.aes = FALSE)

# Re-combine with MiniMap ~
Institutions_PlotUp <- Institutions_Plot + 
  inset_element(MiniMap_Bilingual, left = .329, bottom = .2725, right = .329 + .35, top = .2725 + .35, align_to = "full") +
  plot_layout(guides = "collect")

ggsave("./SBBEPlots/Temp_Institutions_Plot.pdf", 
       plot = Institutions_PlotUp, 
       device = "pdf", 
...
Final_Composite_Plot <- ggdraw(Base_Map) +
  draw_image(
    "./SBBEPlots/Temp_Institutions_Plot.pdf", 
    x = 0.46,       # Centers on Column 2
    y = 0.05,       # Centers on Row 2
    width = 0.42,   # Fixed box size
    height = 0.42
  )
...
# 1. Save standalone circular plot as PNG
ggsave("./SBBEPlots/Temp_Institutions_Plot.png", 
       plot = Institutions_PlotUp, 
       device = "png", 
       width = 6, height = 6, dpi = 600)
...
# Saves Article Map ~
ggsave("./SBBEPlots/Map.pdf", Map,
       device = "pdf", limitsize = FALSE, scale = 1, width = 12.25, height = 6, dpi = 600)


# Combina com os dados dos mapas
combined_data <- bind_rows(subset(fulldf_map, Stats == "Members") %>% mutate(Panel = "Map1"),
                           subset(fulldf_map, Stats == "Attendees") %>% mutate(Panel = "Map2"),
                           empty_panel %>% mutate(Panel = "Empty"))

Map_Grid <- ggplot() +
            geom_sf(data = subset(combined_data, Panel == "Map1"), aes(fill = Percentage * 100), colour = "#f7fbff") +
            geom_sf(data = subset(combined_data, Panel == "Map2"), aes(fill = Percentage * 100), colour = "#f7fbff") +
            geom_blank(data = subset(combined_data, Panel == "Empty")) +
            facet_grid(Panel ~ ., scales = "fixed")


# Converte o Institutions_PlotUp para um grob (objeto gráfico)
Institutions_grob <- ggplotGrob(Institutions_PlotUp)

# Adiciona o grob ao gráfico principal
Map_Grid <- Map_Grid +
            annotation_custom(grob = Institutions_grob,
                              xmin = -75, xmax = -33,
                              ymin = -35, ymax = 6.5)



Map_Grid <- Map_Grid + theme(plot.margin = margin(0, 0, 0, 0))

# Sobrepor o Institutions_PlotUp usando inset_element
Map_Grid_Final <- Map_Grid +
                  inset_element(Institutions_PlotUp,
                                left = 0, bottom = 0.5, right = 0.5, top = 1,
                                align_to = "full")


# Saves Article Map ~
ggsave("./SBBEPlots/Map_Grid_Final.pdf", Map_Grid_Final,
       device = "pdf", limitsize = FALSE, scale = 1, width = 12.25, height = 6, dpi = 600)


map_grob <- ggplotGrob(MiniMap_Bilingual)


Institutions_Plot_Faceted <- ggplot(Circular, aes(x = as.factor(ID), y = Percentage * 100, fill = Region)) +
  
  # Insert map polygon/sf layer mapped to negative y-values (the center hole)
  geom_sf(data = subset(fulldf_map, name_region != "SBBE24"), 
          aes(fill = name_region), colour = "#f7fbff", inherit.aes = FALSE) +
  
  # Main circular barplot layers
  geom_bar(stat = "identity", alpha = 1) +
  scale_fill_manual(values = c("#1b9e77", "#fdb462", "#fb8072", "#bebada", "#80b1d3", "#c994c7")) +
...
# Saves Article Map ~
ggsave("./SBBEPlots/Oi_4.pdf", Institutions_Plot_Faceted,
       device = "pdf", limitsize = FALSE, scale = 1, width = 12.25, height = 6, dpi = 600)


# 1. Build the faceted circular plot WITHOUT the map inset
base_faceted_plot <- ggplot(Circular, aes(x = as.factor(ID), y = Percentage * 100, fill = Region)) +
  geom_bar(stat = "identity", alpha = 1) +
  scale_fill_manual(values = c("#1b9e77", "#fdb462", "#fb8072", "#bebada", "#80b1d3", "#c994c7")) +
  geom_segment(data = base_data_Circular, aes(x = start, y = 5, xend = end, yend = 5), 
               colour = "#000000", alpha = 1, linewidth = .25, linetype = 4, inherit.aes = FALSE) +
  ylim(-100, 80) +
  coord_polar() +
  geom_text(data = label_data_Circular, aes(x = ID, y = Percentage * 100 + 6, label = Variable, hjust = hjust, angle = angle), 
            family = "Cormorant", size = 8, color = "#000000", fontface = "bold", inherit.aes = FALSE) +
  facet_grid(. ~ Stats)

# 2. Draw the base plot, then overlay the map grob at exact relative coordinates per panel
ggdraw(base_faceted_plot) +
  draw_plot(MiniMap_Bilingual, x = 0.18, y = 0.25, width = 0.28, height = 0.28) +  # Panel 1 Center
  draw_plot(MiniMap_Bilingual, x = 0.68, y = 0.25, width = 0.28, height = 0.28)    # Panel 2 Center
...
# Ensure your helper tables retain the grouping variable (e.g., Stats)
base_data_Circular <- Circular %>% 
                      group_by(Stats, Region) %>% 
                      summarize(start = min(ID), 
                      end = max(ID) - empty_bar, 
                      N = n(), .groups = "drop") %>%
                      mutate(end = ifelse(N == 1, start + 1, end))
                      grid_data_Circular <- base_data_Circular %>%
                      group_by(Stats) %>%
                      mutate(end = lag(end, default = last(end)) + 1,
                      start = start - 1)

# Build percentage labels grid per facet panel
perc_annotations <- expand.grid(Stats = unique(Circular$Stats), y = c(5, 10, 15)) %>% 
                    mutate(label = paste0(y, "%"), x = max(Circular$ID))


perc_annotations <- expand.grid(Stats = unique(Circular$Stats), y = c(5, 10, 15)) %>% 
                    mutate(label = paste0(y, "%"), 
                    x = max(Circular$ID[!is.na(Circular$ID)]))

# Base circular plot with facet_grid
Institutions_Plot_Faceted <- ggplot(Circular, aes(x = as.factor(ID), y = Percentage * 100, fill = Region)) +
  geom_bar(stat = "identity", alpha = 1) +
  scale_fill_manual(values = c("#1b9e77", "#fdb462", "#fb8072", "#bebada", "#80b1d3", "#c994c7")) +
  ...
  # Facet-aware percentage text
  geom_text(data = perc_annotations, aes(x = x, y = y, label = label), 
            family = "Cormorant", size = .1, fontface = "bold", color = "#000000", hjust = 1, inherit.aes = FALSE) +
  geom_bar(stat = "identity", alpha = .5) +
  ylim(-100, 80) +
  theme(panel.background = element_rect(fill = "#ffffff"),
        panel.grid = element_blank(),
        panel.border = element_blank(),
        legend.position = "none",
        strip.background = element_rect(fill = "#d6d6d6", colour = "#000000"),
        strip.text = element_text(family = "Cormorant", size = 20, face = "bold"),
        axis.text = element_blank(),
        axis.title = element_blank(), 
        axis.ticks = element_blank()) +
  coord_polar() +
  
  # Dynamic angled labels
  geom_text(data = label_data_Circular, aes(x = ID, y = Percentage * 100 + 6, 
                                            label = Variable, hjust = hjust, angle = angle), family = "Cormorant", size = 2, 
            color = "#000000", fontface = "bold", inherit.aes = FALSE) +
  geom_segment(data = base_data_Circular, aes(x = start, y = -5, xend = end, yend = -5), 
               colour = "#000000", alpha = 1, size = .6, inherit.aes = FALSE) +
...
# 3. Merges Faceted Plot with MiniMap ~
Institutions_PlotUp_Faceted <- Institutions_Plot_Faceted + 
  inset_element(MiniMap_Bilingual, left = .329, bottom = .2725, right = .329 + .35, top = .2725 + .35, align_to = "full") +
  plot_layout(guides = "collect")
